...
namespace doan_1.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public ActionResult SearchLoaiSach(string loaiSach)
        {
            var loaiSachs = from m in db.Category select m;

            if (!String.IsNullOrEmpty(loaiSach))
            {
                loaiSachs = loaiSachs.Where(s => s.CateName.Contains(loaiSach));

            }
            return View(loaiSachs);
